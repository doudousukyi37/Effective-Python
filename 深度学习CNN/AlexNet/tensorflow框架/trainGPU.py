...
import glob
import random
os.environ["CUDA_DEVICE_ORDER"]="PCI_BUS_ID" #将gpu按pci线顺序排列
...
def main():
    ...
    if gpus:
        try:
            for gpu in gpus:
                # 动态分配gpu内存而不是默认即将内存占满
                tf.config.experimental.set_memory_growth(gpu,True)
        except RuntimeError as e:
            print(e)
            exit(-1)    # 程序将退出并返回状态码-1
    ...
    def process_path(img_path,label):
        ...
        image = tf.io.read_file(img_path) # 读取图像将其作为字符串存储在变量image中
        image = tf.image.decode_jpeg(image) # 将jpeg格式的图像解码为一个三维张量
        image = tf.image.convert_image_dtype(image,tf.float32)
        # 将图像张量的数据类型从默认的整型转换为浮点数类型。为了后续的归一化处理，使得像素值在0到1之间
    ...
    # repeat 使数据集无限重复
    # batch 将数据集中的元素分批处理
    # prefetch 下一批数据可以在当前批处理的同时预先加载，从而提高效率
